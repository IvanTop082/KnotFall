...
export class TuringRenderer {
  ...
  setMvps(nodes: TuringNode[], edges: TuringEdge[], edgeScale: number) {
    ...
    // Distance from a node's center to the point along `dir` where the line
    // should start/end, so it sits just outside the node shape.
    ...
    edges.forEach((e, i) => {
      ...
      if (!e.color && (!a.isPrimary() || !b.isPrimary())) {
      ...
      // `direction` points from target to source. Shorten both ends so the
      // line lands just outside each node boundary.
